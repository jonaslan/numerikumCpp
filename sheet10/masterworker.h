...
class Master
{
	...
	MPI_Datatype result_dt; // it works like kind of a template: template<type T> ~ send(...., result_dt, ...), where result_dt ~ type T
	int MPI_Count;
...
int Master::listen(int *command)
{
	MPI_Status status;
	int flag;
	MPI_Iprobe(MPI_ANY_SOURCE, MW_listen_tag, MPI_COMM_WORLD, &flag, &status); // I think it returns some worker who is waiting for sending a message with his results back to the master
	if (flag)
	{
...
// **********
void Master::get_result(int worker_rank, void *data) // data is a void pointer. It points any type but there is a counterpart which I can't remember right now. no pointer arithmetic?
{
	MPI_Status status;
...
bool Worker::get_work(void *data)
{
	...
	MPI_Get_count(&status, MPI_INT, &count); //What's this count for? Just check we don't get an empty argument?Yes,checks number of entries received in previous call(encoded in status:
...
void Worker::send_result(void *data)
{
	...
	MPI_Send(&command, 1, MPI_INT, 0, MW_listen_tag, MPI_COMM_WORLD); //What's the difference between this line and below, method Worker::done() ??? The difference is the message passed
