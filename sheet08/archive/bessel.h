...
class Bessjy
{
	...
	public:
// 	Bessel functions J for special n
	double j0(const double x);
	double j1(const double x);
	
// 	Bessel functions Y for special n
	double y0(const double x);
	double y1(const double x);

// 	Bessel functions J and Y for arbitrary n
...
class Bessik
{
	...
	double poly(const double *cof, const int n, const double x);

	public:
// 	Modified Bessel functions I for special n
	double i0(const double x);
	double i1(const double x);

// 	Modified Bessel functions K for special n
	double k0(const double x);
	double k1(const double x);

// 	Modified Bessel functions I and K for arbitrary n
...
// **********
double Bessjy::jn(int n, const double x)
{
	const double ACC=160.;
	...
	ax=abs(x);
	if (ax*ax <= 8.*numeric_limits<double>::min())
		return 0.;
...
// **********
double Bessik::poly(const double *cof, const int n, const double x)
{
	double ans = cof[n];
