...
int main ()
{
	...
	// output objects for the unusable Dormand-Prince and Stoermer methods		
	//Output out1(1000);
	//Output out2(1000);
	
	// Output objects for the stiff system solvers
	Output out3(1000);
	Output out4(1000);
	...
	// Instantiation of four (two, really) solver objects, one for each method 
	...
	*/
	...
	/* There is no point taking the time, it's whatever is before infinity
...
//	int steps_Stoerm = out2.steps[0];
	...
	cout << "Ross: " << (rossend-rossstart)/double(CLOCKS_PER_SEC) << " steps: " << steps_Ross << 
	"\nSemi-explicit Euler: " << (sieend-siestart)/double(CLOCKS_PER_SEC) << " steps: " << steps_Sie << endl;
